...
1. 分别读取 `库存分析看板源数据.xlsx`、`2026年设备发货明细-共享版.xlsm`（兼容带空格的同名文件）、`物料运费分析.xlsx`、`售后跟进.xlsx` 和 `物料开发进度跟进表.xlsx`，不再依赖 `设备分析.xlsx` 或 `物料及设备报表集合.xlsx`。
...
- 进销存看板：`库存分析看板源数据.xlsx` + `2026年设备发货明细-共享版.xlsm`。
